...
【Highest Priority Raw Diagram Rule】

For complete circuit diagrams, complete PLC ladder diagrams, and complete ASCII diagrams, exact raw-copy preservation has the highest priority.

If any other layout rule conflicts with raw-copy preservation, follow raw-copy preservation.

Do not trade diagram accuracy for OLED compactness.
Do not trade diagram accuracy for beauty.
Do not trade diagram accuracy for shorter height.
Do not process complete diagrams semantically.

For complete diagrams:
- Do not do layout.
- Do not do formatting.
- Do not do reasoning.
- Do not do correction.
- Do not do beautification.
- Do not do redrawing.
- Only wrap each original diagram line as a JSON string inside diagram.lines.

【Diagram / Circuit / PLC Layout Rules】

There are two different diagram-like contents:

A. FULL RAW DIAGRAM

Includes:
- complete electrical control circuit diagram
- complete control circuit diagram
- complete relay-contactor ASCII circuit diagram
- complete PLC ladder diagram
- complete PLC program diagram
- complete hydraulic control ASCII diagram
- any complete text diagram that depends on spaces, indentation, horizontal lines, vertical lines, or alignment
- any ```txt code block``` that contains a complete circuit / PLC / control diagram

For FULL RAW DIAGRAM:
You must use raw-copy mode.

Raw-copy mode means:
- Do not understand the diagram.
- Do not analyze the diagram.
- Do not redraw the diagram.
- Do not improve the diagram.
- Do not normalize the diagram.
- Do not fix the diagram.
- Do not split the diagram unless ANSWER_FULL already clearly provides separate diagram code blocks.
- Do not remove blank lines.
- Do not remove leading spaces.
- Do not remove trailing spaces if they affect alignment.
- Do not add spaces.
...
- Do not replace vertical bars.
- Do not replace slashes or backslashes.
- Do not replace any character.

Your only task for a FULL RAW DIAGRAM is:
1. find the original diagram code block in ANSWER_FULL,
2. remove only the outer Markdown code fence markers such as ```txt and ```,
3. put each original line into diagram.lines,
4. escape JSON characters correctly.

Each line in diagram.lines must be the same visible line as in ANSWER_FULL.

The displayed diagram must be identical to the input diagram.
The diagram content is not something to solve, rewrite, repair, optimize, normalize, or beautify.
It is already finished in ANSWER_FULL.

B. BRANCH / LOGIC / EXPLANATION CONTENT

Includes:
- [BRANCHES]
- logic simplification diagram
- branch expressions
- action explanation
- protection explanation
- component explanation
- short text circuit relationships that do not rely on large ASCII alignment

For these contents:
You may format them into text / diagram blocks for OLED readability.

If ANSWER_FULL contains any diagram-like content, use diagram block for content that depends on spaces, indentation, horizontal lines, vertical lines, or alignment.

Do not convert complete diagrams into normal text blocks.
Do not convert complete diagrams into formula blocks.
Do not rewrite complete diagrams into natural language.
...
For every diagram block, you must count the exact number of strings in diagram.lines.

Then set:
height = diagram.lines.length × 16

Examples:
- 1 diagram line -> height = 16
- 3 diagram lines -> height = 48
- 10 diagram lines -> height = 160
- 18 diagram lines -> height = 288
- 44 diagram lines -> height = 704

Do not guess diagram height.
Do not reuse height from examples.
Do not estimate visually.
Do not use 32, 64, or 96 by default for diagram blocks.
For diagram blocks, height must always equal the number of lines multiplied by 16.

If the diagram contains blank lines:
- For FULL RAW DIAGRAM, preserve blank lines exactly as they appear in ANSWER_FULL.
- Do not remove blank lines inside a full diagram block.
- Only remove Markdown code fence markers such as ```txt and ```.
- For branch logic and explanation content, unnecessary blank lines may be removed.
...
Never output "*" inside a diagram line unless "*" already appears in that exact input line.
...
For FULL RAW DIAGRAM, do not split the diagram by yourself.
Only keep multiple diagram blocks if ANSWER_FULL already provides multiple separate diagram code blocks.
If ANSWER_FULL provides one complete diagram code block, output it as one diagram block.
Never split in the middle of connected wires.
Never reorganize a complete circuit diagram into a new main-circuit/control-circuit layout.
...
- Before final output, recount the number of strings inside lines and verify that height equals lines.length × 16.
- If height does not match lines.length × 16, fix height before output.
