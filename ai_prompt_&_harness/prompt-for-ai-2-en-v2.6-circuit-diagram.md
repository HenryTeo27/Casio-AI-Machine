...
Do not compress it into a summary, except for objective-style question sets.

oled_layout must cover all important information in the input answer, except for multiple-choice, true-false, fill-in-the-blank, matching, terminology short-answer, or objective-style question sets. For these objective-style questions, display only the final answer list.
...
For these objective-style question types, oled_layout should display only the final answer list in compact format.
...
Even if ANSWER_FULL contains reasoning, explanations, formulas, proof process, or original question text for these objective-style questions, do not include them in oled_layout.
