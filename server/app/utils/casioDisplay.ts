...
const RENDER_LATEX_BITMAPS =
    safeText(process.env.CASIO_AI_RENDER_LATEX_BITMAPS).toLowerCase() ===
    "true";
...
function latexToOledMathText(latex: string): string {
    ...
    output = output.replace(/\\sum_\{([^{}]+)\}\^\{([^{}]+)\}/g, "Σ $1..$2");
    output = output.replace(/\\prod_\{([^{}]+)\}\^\{([^{}]+)\}/g, "Π $1..$2");
    output = output.replace(
        /\\binom\s*\{([^{}]+)\}\s*\{([^{}]+)\}/g,
        "C($1,$2)"
    );
...
function containsLatexCommand(value: string): boolean {
    return /\\(?:boxed|d?frac|sqrt|sum|prod|binom|begin|end|left|right|text|leq?|geq?|neq|approx|times|cdot|lambda|alpha|beta|gamma|theta|rho|sigma|mu|pi|int|lim|ln|log|exp)\b/u.test(
        safeText(value)
    );
}

function containsCjk(value: string): boolean {
    return /[\u3400-\u9fff]/u.test(safeText(value));
}

function stripFormulaWrappers(value: string): string {
...
async function rasterSingleLineTextToXbm(args: {
    ...
    let { data, info } = await sharp(rendered.data, {
        ...
        .extend({
            top: Math.max(
                0,
                Math.floor((OLED_HEIGHT - rendered.info.height) / 2)
            ),
            bottom: Math.max(
                0,
                OLED_HEIGHT -
...
async function rasterFallbackFormulaText(
    latex: string
...
async function rasterFormulaTextPages(
    latex: string
): Promise<CasioDisplayBlock[]> {
...
async function rasterTextToXbm(
    ...
    const renderedLines = await Promise.all(
        lines.map((line) => rasterTextLine(line))
    );
    const bitmapWidth = fixedWidth
        ? OLED_WIDTH
        : Math.ceil(
            ...
            return {
                ...
                top:
                    lines.length === 1
                        ? Math.max(
                              0,
                              Math.floor((OLED_HEIGHT - line.height) / 2)
                          )
...
function wrapOledFallbackLines(lines: string[]): string[] {
    return lines.flatMap((line) =>
        wrapLineByUnits(line, OLED_FIXED_PAGE_UNIT_LIMIT)
    );
}
...
function hasNoLayoutLatex(value: unknown): boolean {
    return value === null || value === undefined || safeText(value) === "";
}

function normalizeTextLayoutBlock(block: CasioOledLayoutBlock): string[] {
    if (Number(block.height) !== OLED_HEIGHT) return [];
    if (!hasNoLayoutLatex(block.latex)) return [];

    const rawLines = Array.isArray(block.lines)
        ? block.lines.map((line) => safeText(line)).filter(Boolean)
        : [];
    if (rawLines.length < 1 || rawLines.length > TEXT_LINES_PER_PAGE) {
        return [];
    }
...
function normalizeFormulaLayoutBlock(
    block: CasioOledLayoutBlock
): { latex: string; height: 16 | 32 | 64 } | null {
    const height = normalizeLayoutHeight(block.height, OLED_HEIGHT);
    if (Number(block.height) !== height) return null;
    if (block.lines !== null && block.lines !== undefined) return null;
...
function displayTextFromLayout(layout: unknown): string {
    ...
    for (const rawBlock of layout) {
        if (rawBlock && typeof rawBlock === "object") {
            ...
            if (type === "text") {
                lines.push(...normalizeTextLayoutBlock(block));
            } else if (type === "formula") {
                const formula = normalizeFormulaLayoutBlock(block);
                if (formula) lines.push(latexToOledMathText(formula.latex));
            }
        }
        if (lines.join("\n").length >= 220) break;
    }
...
export async function buildDisplayBlocksFromOledLayout(
    ...
    const renderJobs: Array<
        Promise<CasioDisplayBlock | CasioDisplayBlock[] | null>
    > = [];
    ...
    for (const rawBlock of layout) {
        if (rawBlock && typeof rawBlock === "object") {
            ...
            if (blockType === "text") {
                const lines = normalizeTextLayoutBlock(block);
                if (lines.length) {
                    acceptedBlocks += 1;
                    renderJobs.push(
                        rasterTextToXbm(lines.join("\n"), "text", true)
                    );
                }
                ...
                if (formula) {
                    acceptedBlocks += 1;
                    renderJobs.push(
                        rasterFormulaLayoutToXbm(formula.latex, formula.height)
                    );
                }
            }
        }
    ...
    const output = rendered.filter(
        (item): item is CasioDisplayBlock =>
            item !== null &&
            item.width > 0 &&
            (item.height === 16 || item.height === OLED_HEIGHT || item.height === 64)
    );
...
function collectInputBlocks(rawBlocks: unknown): RawModelBlock[] {
    ...
    for (const item of rawBlocks) {
        if (item && typeof item === "object") {
            const rawType = safeText(
                (item as RawModelBlock).type
            ).toLowerCase();
            if (rawType === "formula") {
...
function isTransitionOnlyText(text: string): boolean {
    ...
    return (
        knownTransition ||
        (/[:：]\s*$/u.test(safeText(text)) &&
            countVisualUnits(normalized) <= 16)
    );
}
...
function answerTextToInputBlocks(answer: string): RawModelBlock[] {
    const lines = safeText(answer)
        .replace(/\r/g, "")
        .split("\n")
        .map((line) => line.trim())
        .filter(Boolean);
    const blocks: RawModelBlock[] = [];

    for (const line of lines) {
        if (!containsLatexCommand(line)) {
            blocks.push({ type: "text", text: line });
        } else if (containsCjk(line)) {
            blocks.push({ type: "text", text: latexToOledMathText(line) });
        } else {
            blocks.push({
                type: "formula",
                latex: stripFormulaWrappers(line)
            });
        }
    }
...
export async function buildDisplayBlocksFromModel(args: {
    ...
    const startedAt = Date.now();
    let parsedBlocks = prepareInputBlocks(args.blocks);
    if (!parsedBlocks.length) {
        const answerBlocks = answerTextToInputBlocks(args.answer || "");
        parsedBlocks = prepareInputBlocks(answerBlocks);
        console.log(`${LOG_PREFIX} answer_fallback_prepare`, {
            sourceBlocks: answerBlocks.length,
            preparedBlocks: parsedBlocks.length,
            answerChars: safeText(args.answer).length
        });
    }
    if (!parsedBlocks.length) {
        const readableAnswer = latexToOledMathText(args.answer || "");
        if (readableAnswer) {
            parsedBlocks = [{ type: "text", text: readableAnswer }];
            console.warn(`${LOG_PREFIX} readable_text_fallback_prepare`, {
                answerChars: safeText(args.answer).length,
                readableChars: readableAnswer.length
            });
        }
    }
    if (!parsedBlocks.length) {
        const fallbackPages = splitTextIntoPages(latexToOledMathText(args.answer || ""));
    ...
    for (const block of parsedBlocks) {
        if (block.type === "formula") {
            ...
            for (const page of textPages) {
                renderJobs.push(
                    rasterTextToXbm(page, "text").then((item) => [item])
                );
            }
        }
    }
