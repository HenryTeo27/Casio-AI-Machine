...
function latexToOledMathText(latex: string): string {
  ...
  output = output.replace(/\\frac\s*\{([^{}]+)\}\s*\{([^{}]+)\}/g, "($1)/($2)");
...
async function rasterTextToXbm(
  ...
  const bitmapWidth = fixedWidth
    ? OLED_WIDTH
    : Math.ceil(
        Math.max(OLED_WIDTH, ...renderedLines.map((line) => line?.width || 0)) /
          8,
      ) * 8;
...
export async function buildDisplayBlocksFromOledPages(
  ...
  for (const rawPage of pages) {
    if (rawPage && typeof rawPage === "object") {
      ...
      for (let index = 0; index < lines.length; index += TEXT_LINES_PER_PAGE) {
  ...
  const output = rendered.filter(
    (item): item is CasioDisplayBlock =>
      item !== null && item.width === OLED_WIDTH && item.height === OLED_HEIGHT,
  );
...
export async function buildDisplayBlocksFromOledLayout(
  ...
  const output = rendered.filter(
    (item): item is CasioDisplayBlock =>
      item !== null &&
      item.width > 0 &&
      (item.height === 16 || item.height === OLED_HEIGHT),
  );
